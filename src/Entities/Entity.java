 ...
 */
public abstract class Entity implements MapEntity{
    ...
    public void updateMovement() {
        ...
        if (completed == 0)
            getNextPosition(currentDirection);
    ...
    public void hit(int damage) {
        ...
        if (!hurt) {
            ...
            hurtTime = 50;
            undoWalking();
            getNextPosition(getOppositeDirectionOf(facingDirection));
        }
        hurt = true;
    }
    /**
    ...
    public void setBlocks(ArrayList<MapEntity> blocksToSet) {
        blocks = blocksToSet;
    }
    ...
    // meant if you just want to see what is right next to something
    public Rectangle getFacingRect(){
        switch (facingDirection) {
            ...
            case Right:
                return new Rectangle(position.x + position.width, position.y, 5, position.height);
        }
        return null;
    }
    /**
     ...
     */
    protected void getNextPosition(Direction dir) {
        ...
        if (newPosition != null) {
            ...
            for (MapEntity block : blocks){
                if (collisionCondition(block, newPosition)){
                    colliding = true;
                    return;
                }
            }
    ...
    public Direction face(Rectangle pos) {
    ...
    public double distance(Rectangle other) {
        return Math.pow(other.x - position.x, 2) + Math.pow(other.y - position.y, 2);
    }
